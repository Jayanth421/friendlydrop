...
// All routes in admin dashboard require admin authentication
const PROTECTED_PREFIXES = ["/dashboard", "/users", "/products", "/orders", "/analytics", "/settings"];
...
export function middleware(request: NextRequest) {
  ...
  if (needsAuth && !session) {
    // Redirect to main website login with return URL to admin subdomain
    const loginUrl = new URL("https://friendlydrop.in/login");
    loginUrl.searchParams.set("redirect", `https://admin.friendlydrop.in${pathname}`);
